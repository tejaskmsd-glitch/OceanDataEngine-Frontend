...
def build_artifact_store() -> ArtifactStore:
    ...
    if s.in_memory or s.endpoint_url is None:
        return InMemoryArtifactStore(bucket=s.artifact_bucket)
    return S3ArtifactStore(bucket=s.artifact_bucket, endpoint_url=s.endpoint_url, region=s.region, access_key=s.access_key, secret_key=s.secret_key)
